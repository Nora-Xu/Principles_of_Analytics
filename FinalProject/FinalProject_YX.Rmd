...
**We've also uploaded the raw file "DATA Binding.xlsx" along with this submission.**
...
## Accuracy:
**For categorical variables, we applied factor function to "gender" and "condition".**

```{r accuracy}
summary(my_data)
notypos <- my_data
str(notypos)
##Factor the character vaiables
...
**We use the 5% rule to exclude rows that have missing data.**\
**And the result shows we do not have missing data.**
```{r missing}
percentmiss <- function(x) { sum(is.na(x))/length(x) * 100}
#checking the missing value for each row
missing <- apply(notypos, 1, percentmiss)
table(missing)
```
## Outliers:
**We removed rows from raw data where participant are marked as "excluded" due to failing the practice and attention problems( Exclude..1..yes = 1)**\
**Also, we only included the data for trails between 9 to 32, which are the experimental trials.** 

```{r outliers}
##remove rows when excluded..1..yes == 1,which participant excluded due to failing practice and attention problems
...
**The dots within the (-2, 2) range are along the linear line. Linearity assumption met.**\
...
## Normality:
**According to the histogram, the distribution looks close to normal. Normality assumption met.**
...
**The spread above 0 and below in both horizontal and vertical directions are relatively equal. The spread across X axis is quite even. Assumption met.**\
```{r homog-s}
...
**We grouped each subject (participant) to calculate their average accuracy score.**\

```{r}
##we use dplyr package function to create a new data set, witch including SujNum, Condition, and Average points of the accuracy for each child.
dat <- mydata %>% 
  group_by(SubjNum, Condition) %>%
  dplyr::summarize(avgAcc = mean(Accuracy.SelectedTarget.))
head(dat)
```

## Part 3: The Analysis
**We decided to use T-Test to test our null hypothesis.**
...
The p-value is 0.1321 which is bigger than our critical value .05, so we failed to reject the null that the type of language condition (directional/relational) has no influence on children's accuracy in conjuncting color and space. This conclusion has the opposite direction with both the original study and the replicate.\

With a sample size of 48 subjects, our effect size is 0.4721827, which is small.\
